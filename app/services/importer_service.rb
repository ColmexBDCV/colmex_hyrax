...
module ImporterService
    ...
    def validate_csv(sip, work, repnal = false)
        ...
        records.each_with_index do |record,index|
            ...
            report["Los registros que se encuentran en las siguientes Filas tienen un ISBN que ya existe en el sistema  (campo isbn)"] = (report["Filas que contienen un registro ya existente en el sistema (mismo isbn)"] || []) << [index + 2, [record.isbn]] if record.respond_to?("isbn") && wt.where(isbn: record.isbn).count > 0
